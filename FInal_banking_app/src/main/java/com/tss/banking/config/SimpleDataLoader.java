...
public class SimpleDataLoader implements CommandLineRunner {
	...
	private void createFreshAdminUsers() {
		...
		if (!adminRepository.existsByEmail("admin@bank.com")) {
			Admin superAdmin = Admin.builder().firstName("Super").lastName("Admin").email("admin@bank.com")
					.passwordHash(passwordEncoder.encode("admin123"))
					.roles(Set.of(AdminRole.SUPER_ADMIN, AdminRole.ADMIN)).active(true).build();
		...
		if (!adminRepository.existsByEmail("user@bank.com")) {
			Admin regularAdmin = Admin.builder().firstName("Regular").lastName("Admin").email("user@bank.com")
					.passwordHash(passwordEncoder.encode("user123")).roles(Set.of(AdminRole.ADMIN)).active(true)
					.build();
