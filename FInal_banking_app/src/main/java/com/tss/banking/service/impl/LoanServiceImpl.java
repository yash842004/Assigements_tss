...
public class LoanServiceImpl implements LoanService {
	...
	public LoanResponseDTO approveLoan(Long loanId, Long adminId, LoanApprovalRequestDTO request) {
		...
		// Update loan status to approved and active (immediate disbursement)
		loan.setStatus(LoanStatus.ACTIVE);
		loan.setApprovalDate(LocalDateTime.now());
		loan.setDisbursementDate(LocalDateTime.now()); // Immediate disbursement
		loan.setApprovedBy(admin);
		loan.setApprovalNotes(request.getApprovalNotes());
		loan.setLastUpdated(LocalDateTime.now());

		// Automatically disburse loan amount to account
		...
		// Send loan approval and disbursement notification email
	...
	public TransactionResponseDTO processEMIPayment(EMIPaymentRequestDTO request) {
		...
		// Check account balance for automatic deduction
		if (request.getIsAutomaticDeduction() && account.getBalance().compareTo(request.getAmount()) < 0) {
			// Apply late fee for insufficient balance
			...
			throw new InsufficientBalanceException(
					"Insufficient balance for EMI payment. Late fee of ₹" + lateFee + " applied.");
		}

		// Check balance for manual payment
		if (!request.getIsAutomaticDeduction() && account.getBalance().compareTo(request.getAmount()) < 0) {
			throw new InsufficientBalanceException("Insufficient balance for loan payment");
		}
		...
		// Send EMI deduction notification email
		Customer customer = loan.getCustomer();
		...
		for (Loan loan : overdueLoans) {
			try {
				...
				if (overdueDays > 0) {
					...
					// Apply late fee only once every 30 days
					if (overdueDays % 30 == 0) {
						BigDecimal lateFee = calculateLateFee(loan.getId());
						loan.setLateFeeAmount(loan.getLateFeeAmount().add(lateFee));
						loan.setTotalLateFees(loan.getTotalLateFees().add(lateFee));
						...
						log.info("Applied late fee of ₹{} to loan ID: {} (overdue by {} days)", lateFee, loan.getId(),
								overdueDays);
					}
	...
	public BigDecimal calculateLateFee(Long loanId) {
		Loan loan = findLoanById(loanId);
		// Calculate late fee as 2% of monthly payment or minimum ₹100
