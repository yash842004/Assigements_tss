...
public class LoanEligibleTest {
	...
	public static void main(String[] args) {
		...
		LoanEligible person1 = new LoanEligible("Om", 89, 53000, 650);
		...
		Predicate<LoanEligible> isEligible = (persionP) -> {
			boolean condition;
			if ((persionP.getAge() >= 21) && (persionP.getIncome() > 25000) && (persionP.getCreditScore() >= 650)) {
				condition = true;
		...
		if (isEligible.test(supplier)) {
			Function<LoanEligible, Integer> amountToGet = (result) -> {
				...
				float amount = (float) (person1.getIncome() * (person1.getCreditScore() / 1000.0));
