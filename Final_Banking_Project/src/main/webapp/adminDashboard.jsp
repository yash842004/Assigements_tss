...
.action-btn.history:hover, .action-btn.activate:hover {
	color: var(--primary-color);
}
					...
					<tbody>
						<c:forEach var="custView" items="${customers}">
							<tr>
								...
								<td><span
									class="status-badge status-${custView.status.toLowerCase()}">${custView.status}</span>
								</td>
								<td class="text-center"><c:if
										test="${custView.status == 'PENDING'}">
										<a href="approveCustomer?id=${custView.customerId}"
											class="action-btn approve" title="Approve"><i
											class="bi bi-check-circle-fill fs-5"></i></a>
										<a href="rejectCustomer?id=${custView.customerId}"
											class="action-btn reject" title="Reject"><i
											class="bi bi-x-circle-fill fs-5"></i></a>
									</c:if> <c:if test="${custView.status == 'APPROVED'}">
										<a href="deactivateCustomer?id=${custView.customerId}"
											class="action-btn deactivate" title="Deactivate"><i
											class="bi bi-slash-circle-fill fs-5"></i></a>
										<a href="viewTransactions?id=${custView.customerId}"
											class="action-btn history" title="History"><i
											class="bi bi-clock-history fs-5"></i></a>
									</c:if> <c:if test="${custView.status == 'DEACTIVATED'}">
										<a href="activateCustomer?id=${custView.customerId}"
											class="action-btn activate" title="Activate"><i
											class="bi bi-arrow-repeat fs-5"></i></a>
									</c:if></td>
							</tr>
						</c:forEach>
					</tbody>
